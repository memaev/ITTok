...
import 'package:tiktok_for_it/features/auth/presentation/screens/login_screen.dart';
import 'package:tiktok_for_it/features/auth/presentation/screens/register_screen.dart';
...
/// Creates the [GoRouter] instance wired to Riverpod auth state.
/// Called from [routerProvider] so the router is recreated if needed.
GoRouter createAppRouter(Ref ref) {
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      // While Firebase resolves the initial auth state, do not redirect.
      if (isLoading) return null;

      // Unauthenticated users may only visit auth routes.
      if (!isLoggedIn && !isAuthRoute) return Routes.login;

      // Authenticated users should not land on auth routes.
      if (isLoggedIn && isAuthRoute) return Routes.home;

      return null;
    },
    routes: [
      // Auth routes — outside ShellRoute so they render without bottom nav.
      GoRoute(
        path: Routes.login,
...
// Bridges a Stream into a ChangeNotifier so GoRouter re-evaluates its redirect
...
class _AppShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: child,
      bottomNavigationBar:
          isTopicFeed ? null : _BottomNav(currentLocation: location),
    );
  }
}
